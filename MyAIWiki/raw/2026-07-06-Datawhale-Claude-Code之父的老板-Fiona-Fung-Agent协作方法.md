...
author: 访谈 Fiona Fung，编辑 Datawhale
publish_date: 2026-07-06（公众号推送日期，未抓到精确发布时间）
capture_date: 2026-07-06
source_url: https://mp.weixin.qq.com/s/c9Vkx_2l2PRSY__GjUlHuQ
original_source: https://www.youtube.com/watch?v=Ybrl4FYM57c（Lenny's Podcast 访谈 Fiona Fung）
biz: MzIyNjM2MzQyNg==
mid: 2247724016
sn: bd928a451c6459e3b5fe3eec0be36614
status: inbox
...
- **Lenny's Podcast 访谈 Fiona Fung**：https://www.youtube.com/watch?v=Ybrl4FYM57c
- **Fiona Fung**：Anthropic Claude Code 与 Cowork 工程与产品团队负责人，Boris Cherny（Claude Code 之父）与 Catherine Wu 都向她汇报
- **Datawhale**：公众号编辑团队，负责编译发布本文
...
- **Lenny Rachitsky**：Lenny's Podcast 主持人，访谈者
- **Datawhale**：公众号编辑团队，编译本文

## 关键术语

- **验证取代编写**：核心动作从"做出来"变成"判断做得对不对"，跨角色重复出现的规律
- **spec 入仓**：把"什么算好"写成 spec 检入代码仓库，Claude 做 code review 时对照 spec 校验
- **TDD 回归**：测试驱动开发因 AI 分担执行工作重新变得容易坚持
- **PM 第二大冲击**：PM 不再受限于工程带宽，可自己动手实现
- **数据科学家变审核员**：AI 生成的"半错"分析需要数据科学家不断审核
- **Agency（自主行动力）**：高 Agency 必须绑定高 Accountability
- **犯新的错误**：允许犯错，但每次必须犯的是新错误
- **Bad/Sad 分级**：Bad=严重不可恢复错误；Sad=可恢复但影响体验问题；定义权下放各组
- **别把动作当成进步**：不在衡量工具使用量，衡量的是"行动是否推动想要的结果"
- **超级卖家**：Facebook Marketplace 早期发现的部分地区少数卖家贡献大部分 GMV
- **Claude Code 远程会话**：常驻代码仓库、Slack 频道、仪表盘的 Claude 实例
- **Routines**：Claude Code 的新功能，每天固定时间运行，自动扫描反馈并生成 PR
- **异步化**：从同步写 prompt 等结果，演进到 Routine 自动派发 Agent 异步出 PR
- **切换负荷**：并行 Agent 太多，需要不断检查进度、记住每路任务做到哪一步
- **孤独感**：和 Agent 工作的多、和人工作的少，类似孩子"平行游戏"
- **结伴编程午餐**：用物理并肩缓解孤独感
- **IC 优先**：新晋管理者先以个人贡献者身份工作一段时间
- **双击依赖层**：工程师要花时间深入理解所依赖的代码层才能更好利用变化

...
1. **本工作区 MyAIWiki 已经是 Karpathy Wiki 模式的实操实例**——本知识库用 raw/（原文、digest、notes）+ wiki/ + prompts/ + scripts/ + CLAUDE.md 五层结构，跑通 4 月以来；可对外做"现成 demo"案例
...
- **MyAIWiki 本库** — 是这整套模式的自运行实例（raw/ + wiki/ + log.md + index.md + CLAUDE.md）

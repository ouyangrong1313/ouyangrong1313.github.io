---
title: 更可靠的主播助理:淘宝主播 Agent 的 Harness 工程实战
slug: 阿里云开发者-淘宝主播Agent的Harness工程实战
...
- **会话存 MySQL。** 会话与运行时状态对一致性和点查性能要求最高,我们用 MySQL 的 agent_session 表来承载,以 user_id + session_id + state_key 为索引按需点查加载。一个 state_key 对应一类状态(会话基础信息、场次信息、原始消息、运行时消息、卸载上下文、压缩事件、Plan 规划等), 在 agent 运行过程中按需加载。由于状态全部落在 MySQL 这类共享存储里,而非某个副本的本地内存,Agent 才能做到多副本对等部署、跨节点状态一致、服务不中断。
...
- 常规情况下当 Token 超出压缩阈值时,走 3 层压缩:压缩历史工具调用、摘要历史对话轮次、压缩当前轮次消息;
- 当对话超过 N 轮触发 Session 级话题分段,把多轮对话压缩为结构化摘要,并打上场景标签(pre-live / on-live / post-live 等),支持后续按场景聚合检索。这样既控制了体积,又保留了可回溯的结构。
...
- 需提取相关数据,上下文中读取 fileKey,沙箱执行 shell 命令过滤文本获取
- 只需要摘要,摘要从工具输出中返回
...
2. **Schema 强约束 + 幂等性设计**:所有工具签名和决策输出都通过 JSON Schema 约束,在结构层面杜绝非法参数。更关键的是幂等性:任何有副作用的写操作(改价、切品、发券)都必须携带幂等键(UUID),框架层在执行前做去重校验。框架层维护一个幂等键缓存,这样即使网络重试或 Agent 重新规划导致同一个指令被发了两次,也绝不会出现"双切品""双改价"这种灾难性后果。
...
- **PreReasoning**:注入上下文。把最新 State、当前场景的预加载记忆注入 system prompt,确保模型每一轮都看到完整、最新的决策依据。根据用户 query 按需加载长期记忆。
...
### 2.7 LLM 驱动的 PlanEngine:从 ReAct 单步到 DAG 全局
...
4. **提升执行成功率**:从局部最优升级为全局最优规划;输出做固定规则 + 语义双重 Schema 校验;失败子任务支持自动重试;尤其是**增量 Replan**:某个子任务失败时,只对受影响的后续节点重新规划,而不是把整个计划推倒重来。再加上 Token 额度控制(单任务/单场次设预算上限),防止资源耗尽。
...
基于 Decision Trace Log,我们设计了**信任度(trust_score)更新机制**。trust_score 衡量的是"主播对 Agent 建议的信任程度",播后逐条 trace 归因后立即更新:
...
- 本次正文来自微信公众号"阿里云开发者"推送(阿里妹导读系列)
- 文章未注明确切作者署名,推断为阿里 / 淘宝直播技术团队
- "Harness"六元组形式化命名 (E, T, C, S, L, V) 与本工作区既有 [[harness-engineering]] 概念体系强对应,可与 [[Harness工程AgentLoop]] [[HarnessEngineering企业级实战]] [[0xCodez-Agent-Harness-14-Steps]] 形成"Harness 主线四面"互补
- 文中 PlanEngine vs ReAct 的对比表、审批分层表、信任度更新表都给出了具体数字,落地性极高
- 文中"矛盾累积 ≥ 3 次"是工程取舍,不覆盖 L1 而是累积证据等主播确认,与 [[记忆是-agent-基建]] 主张的"记忆是基础设施"形成显式对话
- 微信 HTML 抽取时第 2.6 节"响应超时"行被截断(原本应有"系统检测"等列),已标注空行

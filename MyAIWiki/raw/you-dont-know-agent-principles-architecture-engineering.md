...
```markdown
### Compact Instructions 如何保留关键信息 保留优先级：
1. 架构决策，不得摘要
2. 已修改文件和关键变更
3. 验证状态，pass/fail
4. 未解决的 TODO 和回滚笔记
...
1. **Tool Search，动态工具发现**：别把全部工具定义一次性塞给模型。Agent 通过 search_tools 按需发现工具定义，上下文保留率可达到 95%，Opus 4 的准确率也从 49% 提升到 74%。
2. **Programmatic Tool Calling，代码编排**：别让中间数据一轮轮穿过模型，而是让模型用代码编排多个工具调用，中间结果在执行环境中流转，不进入 LLM 上下文，token 消耗可从约 150,000 降到约 2,000。
3. **Tool Use Examples，示例驱动**：每个工具附带 1-5 个真实调用示例。JSON Schema 只能描述参数类型，但无法表达调用方式，加入示例后，工具调用准确率可从 72% 提升到 90%。
...
1. **上下文窗口，工作记忆**：当前任务所需的最小信息，token 有限，得主动管理
2. **Skills，程序性记忆**：怎么做某件事，操作流程、领域规范，按需加载不默认常驻
3. **JSONL 会话历史，情景记忆**：发生了什么，磁盘持久化，支持跨会话检索
4. **MEMORY.md，语义记忆**：Agent 主动写入认为重要的事实，每次启动时注入系统提示
...
一说 到多 Agent，不少人先想到的就是并行，但工程上先要解决的其实是隔离和协作，这里对应的是两种完全不同的工作模式。
...
多个 Agent 频繁互动时，错误也会被一层层放大。Agent A 先带偏，Agent B 跟着强化，Agent C 再继续叠加，最后所有 Agent 都收敛到同一个高置信度的错误结论。交叉验证的价值就在这里，它能打断这条链，让某个 Agent 独立判断，而不是顺着前面的结论继续走。
...
这张图里真正需要记住的，其实就三组概念，第一组是 task 任务、trial 单次运行、grader 评分器，分别对应测什么、跑多少次、怎么打分，第二组是 transcript 完整执行记录和 outcome 环境最终结果，评测不能只看其中两边，第三组是 agent harness 被评测的 Agent 运行框架和 evaluation harness 评测基础设施，后者负责把任务跑起来、打分、汇总结果，evaluation suite 就是一批任务的集合，是评测跑起来的原材料。
...
1. **负反馈触发**：用户明确表示不满意的 Trace，100% 进队列
2. **高成本对话**：token 消耗超过阈值的，优先审查，往往代表 Agent 在绕圈子
3. **时间窗口采样**：每天固定时间段随机采，保持对正常流量的覆盖
4. **模型或 Prompt 变更后**：头 48 小时全量审查，确认没有退化
...
async function executeShell(args: string[], cwd?: string): Promise<string> {
  ...
  const result = await execFile(args, args.slice(1), { cwd: workDir, timeout: 30_000 });
...
2. **敏感操作显式确认**：向第三方传信息、调用写操作，执行前必须让用户确认，不能静默执行
...
- [How we rebuilt Next.js with AI in one week](https://blog.cloudflare.com/vinext/) — Cloudflare
- [I ported JustHTML from Python to JavaScript with Codex CLI](https://simonwillison.net/2025/Dec/15/porting-justhtml/) — Simon Willison
- [Introducing Agent Skills](https://claude.com/blog/skills) — Anthropic
